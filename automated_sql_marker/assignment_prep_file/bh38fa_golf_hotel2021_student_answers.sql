...
DROP VIEW bh38fa_golfing_history CASCADE  CONSTRAINTS;  ---------------Renamed to golfing_history to match naming in the question.
DROP TABLE bh38fa_hotel_staff CASCADE CONSTRAINTS;
DROP TABLE bh38fa_booking CASCADE CONSTRAINTS;  -----------------------*1-Missing CONSTRAINTS keyword
DROP TABLE bh38fa_golfer CASCADE  CONSTRAINTS;
DROP TABLE bh38fa_room CASCADE  CONSTRAINTS;
DROP TABLE bh38fa_hotel CASCADE  CONSTRAINTS;

CREATE TABLE bh38fa_hotel  --------------------------------------------*2-"hotel" table not created
...
CREATE TABLE bh38fa_room
(room_numb NUMBER(4) NOT NULL,                                   			
hotel_numb NUMBER(2) NOT NULL REFERENCES bh38fa_hotel(hotel_numb),
room_type VARCHAR2(1),
tariff NUMBER(5,2),
PRIMARY KEY(room_numb, hotel_numb));                            

CREATE TABLE bh38fa_golfer  --------------------------------------------*3-Missing "TABLE" in declaration of "CREATE TABLE" 
...
CREATE TABLE bh38fa_booking
(hotel_numb NUMBER(2) NOT NULL REFERENCES bh38fa_hotel(hotel_numb),
golfer_numb NUMBER(6) NOT NULL REFERENCES bh38fa_golfer(golfer_numb),
