# imports
from project_helper_functions.project_helper_functions import (
    get_file_contents,
    extract_comments,
    get_task_and_question_numbers_to_mark,
    extract_task_question_mark,
    compare_found_marks_comments_expected,
    extract_statements,
    get_explain_plan,
    get_id_prefix,
    strip_identifier,
    find_missing_tasks_and_questions,
)
...
def mark_script(home_dir, sample_file, student_file, display_detail=True):
    ...
    task_question_mark_tuples_list = extract_task_question_mark(
        sample_file_comments, tq_to_mark_dict
    )

    # check that marks have been found in the sample file for all answers(statements) defined for marking
    questions_without_marks = compare_found_marks_comments_expected(
        task_question_mark_tuples_list, tq_to_mark_dict
    )
    ...
    print()
    sample_queries = extract_statements(
        sample_file_contents, task_question_mark_tuples_list
    )
    ...
    student_queries = extract_statements(
        removed_identifier_contents, task_question_mark_tuples_list
    )
    ...
    missing_answers = find_missing_tasks_and_questions(
        sample_answer_objects, student_answer_objects
    )
    if missing_answers:
    ...
    # Marking
    df_cell_compare_result_dict = compare_all_dataframes(
        sample_answer_objects, student_answer_objects
    )
    ...
    for i, answer in enumerate(sample_answer_objects):
        ...
        for j, ans in enumerate(student_answer_objects):
            if ans.task_id == tid and ans.question_id == qid:
                if (
                    sample_answer_objects[i].plan_tree is not None
                    and student_answer_objects[j].plan_tree is not None
                ):
                    print()
                    print(
                        "------------------------------------------------------------------------------"
                    ...
                    print(
                        f"Student df {student_answer_objects[j].task_id}, {student_answer_objects[j].question_id} "
                    )
                    print(student_answer_objects[j].query_text)

                    # print(student_answer_objects[j].display_plan_df())
                    result = compare_trees(
                        student_answer_objects[j].plan_tree,
                    ...
                    if (
                        sample_answer_objects[i].plan_tree is not None
                        and student_answer_objects[j].plan_tree is None
                    ):
                        print("sample has tree, student no tree")
                    elif (
                        sample_answer_objects[i].plan_tree is None
                        and student_answer_objects[j].plan_tree is not None
                    ):
