...
def main():
    ...
    message = tk.Label(
        my_frame, text="Welcome to the SQL Automated Marking System!", foreground="blue"
    )
    message.grid(row=0, column=1, columnspan=3)

    # button to open file dialog to choose marking sample file
    button1 = tk.Button(
        my_frame, text="Select Marking Sample Script", command=lambda: select_files()
    )
    button1.grid(row=1, column=0)

    # Message box to show name of file selected
    message_box1 = tk.Text(
        my_frame,
        height=1,
        width=100,
    )
    ...
    markscr_btn = tk.Button(
        my_frame,
        text="Display Mark Script",
        command=lambda: display_file_contents(marking_script_file_name, full_path=True),
    )
    markscr_btn.grid(row=2, column=2, sticky="E")

    # A button to display the student's script
    studscr_btn = tk.Button(
        my_frame,
        text="Display Student Script",
        command=lambda: display_file_contents(
            file_selection_box.get(), full_path=False
        ),
    )
    ...
    process_btn = tk.Button(
        my_frame,
        text="Mark Script",
        command=lambda: call_marker_script(
            current_directory,
            marking_script_file_name,
            file_selection_box.get(),
            detail_selected.get(),
        ),
    )
    ...
    detail_check_box = tk.Checkbutton(
        my_frame,
        text="Show_Detail",
        variable=detail_selected,
        command=lambda: print(detail_selected.get()),
    )
    ...
    exit_btn = tk.Button(
        my_frame, text=" Exit! ", foreground="red", command=lambda: quit()
    )
    ...
    sys.stdout.write = (
        redirector  # whenever sys.stdout.write is called, redirector is called.
    )
